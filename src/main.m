...
m = 5000;
zz = linspace(-cutoff_lo,4*max(dd)-cutoff_hi,m).';         % linearly spaced points
zzfine = linspace(0,2*max(dd),4*m).';

% exponential clustering layout still WIP
% zz = [                                      % two intervals of exponentially clustered points near cutoff
% expClust(m,a=(cutoff_lo-cutoff_hi)/2,b=cutoff_lo,exponent=1,reverse=true); 
% expClust(m,a=cutoff_lo,b=(cutoff_hi-cutoff_lo)/2,exponent=1);    
% expClust(m,a=(cutoff_hi-cutoff_lo)/2,b=cutoff_hi,exponent=1,reverse=true);    
% expClust(m,a=cutoff_hi,b=2*max(dd),exponent=1);
% ];
...
yr = applyrLx(x,L,pol,res);                  % compute r(L)
...
%%
    ...
    scatter(zzfine, zzfine, ms, LW, lw)
    scatter(zzfine, h(zzfine), 0.5*ms, 'filled', LW, lw)
    % xline(cutoff_idx_lo-1/2,":r",LW,lw)
    % xline(cutoff_idx_hi-1/2,":r",LW,lw)
    xline(cutoff_lo,":r",LW,lw)
    xline(cutoff_hi,":r",LW,lw)
    xlabel("$j$")
    ...
    plot(zzfine,h(zzfine),LW,lw);
    plot(zzfine,rpf(zzfine),LW,lw);
    plot(zzfine,p(zzfine),LW,lw);
    xlim([0 2*max(dd)])
    xlabel("$\lambda$")
    ylabel("Response")
    legend(["Exact" "Rational" "Polynomial"])
    ...
    semilogy(zzfine,abs(h(zzfine)-rpf(zzfine)),LW,lw);
    semilogy(zzfine,abs(h(zzfine)-p(zzfine)),LW,lw);
    xlim([0 2*max(dd)])
    xlabel("$\lambda$")
    ylabel("Error")
    legend(["Rational" "Polynomial"])
